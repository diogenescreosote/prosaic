...
def build(
    ...
    write_thread_bodies(mbox_dir, out_dir / "threads")
    return tsv, md, len(rows)


def write_thread_bodies(mbox_dir: Path, out_dir: Path) -> int:
    """One decoded full-text file per thread, quoted chains included.

    The rendered thread PDFs reproduce Gmail's print view, which
    collapses a quoted reply chain to "[Quoted text hidden]" --- so an
    email that exists in the record only as text quoted inside a later
    message is invisible to any search of the rendered text. The stored
    mbox has the full MIME body, but it is base64- or
    quoted-printable-encoded, so grepping the .mbox file finds nothing
    either. This writes the decoded bodies out so the whole of what the
    mailbox holds is searchable.
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    written = 0
    for mb in sorted(mbox_dir.glob("*.mbox")):
        try:
            box = mailbox.mbox(str(mb))
        except Exception as exc:
            print(f"mail-index: skipping {mb.name}: {exc}", file=sys.stderr)
            continue
        parts = [
            f"[[[ decoded thread bodies: {mb.stem} ]]]",
            "Generated by `sc mail-index` from the stored mbox; regenerate, never edit.",
            "FULL decoded bodies, including quoted reply chains that the rendered PDF",
            "collapses to \"[Quoted text hidden]\". Machine text: verify against the",
        ...
        for i, msg in enumerate(box, start=1):
            parts += [
                ...
                f"To: {msg.get('To', '')}",
                f"Cc: {msg.get('Cc', '')}",
                f"Subject: {_WS_RE.sub(' ', str(msg.get('Subject') or '')).strip()}",
                "",
                _body_text(msg).replace("\r", "").strip(),
                "",
            ]
        (out_dir / f"{mb.stem}.txt").write_text("\n".join(parts), encoding="utf-8")
        written += 1
    return written
