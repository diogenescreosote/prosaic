...
def matter(tmp_path: Path) -> Path:
    ...
    subprocess.run(["git", "-c", "user.email=t@example.com", "-c", "user.name=t", "commit", "-q", "--allow-empty", "-m", "chore: init"], cwd=m, check=True)
    return m
...
def test_agent_run_role_reads_matter_yaml(matter: Path, tmp_path: Path):
    bin_dir = tmp_path / "bin"; bin_dir.mkdir(); fake_claude(bin_dir)
    env = {**os.environ, "PATH": f"{bin_dir}:{os.environ['PATH']}", "PROSAIC_AGENT_CLI": "claude"}
    out = subprocess.run([str(AGENT_RUN), "--role", "triage"], input="hello", text=True, cwd=matter,
                         capture_output=True, env=env).stdout
    assert "--model claude-test-model" in out and "--max-turns 7" in out
    out2 = subprocess.run([str(AGENT_RUN), "--role", "triage", "--model", "explicit"], input="hello", text=True,
                          cwd=matter, capture_output=True, env=env).stdout
    assert "--model explicit" in out2, "an explicit flag wins over the role"
    out3 = subprocess.run([str(AGENT_RUN), "--role", "nosuch"], input="hello", text=True, cwd=matter,
                          capture_output=True, env=env).stdout
    assert "--model" not in out3, "an unknown role adds nothing"


def test_watch_mode_triages_settled_inbox_files_and_writes_a_summary(matter: Path, tmp_path: Path):
    inbox = matter / "inbox"
    ...
    proc = subprocess.run(["/bin/bash", str(SYNC), str(matter), "--watch"], env=env, capture_output=True, text=True, timeout=600)
    assert proc.returncode == 0, proc.stderr
    ...
    seen.unlink()
    proc2 = subprocess.run(["/bin/bash", str(SYNC), str(matter), "--watch"], env=env, capture_output=True, text=True, timeout=600)
    assert proc2.returncode == 0 and not seen.exists()


def test_refine_inputs_and_standup_agenda(matter: Path):
    (matter / "QUESTIONS.md").write_text("# QUESTIONS\n\n1. When did the letter arrive?\n")
    (matter / ".state").mkdir(exist_ok=True)
    (matter / ".state" / "sync_last_run.json").write_text(json.dumps({
        "mode": "scheduled", "started": 1, "finished": 2, "outcome": "connector-failures",
        "new_files": 3, "triage": "ok", "connectors": {"gmail": "ok", "mycase": "failed"},
    ...
    assert "## Commits" in inputs and "## Text coverage" in inputs and "failed connectors: mycase" in inputs
...
def test_clean_older_than_reports_only_disposable_trees(matter: Path):
    (matter / "out" / "env").mkdir(parents=True)
    old = time.time() - 40 * 86400
    for rel in ("out/env/old.pdf", "derived/find/old.md", "assets/keep.pdf", "out/env/new.pdf"):
        p = matter / rel; p.parent.mkdir(parents=True, exist_ok=True); p.write_text("x")
        if "new" not in rel:
            os.utime(p, (old, old))
