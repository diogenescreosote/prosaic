...
is the development contract, whatever harness you are: it is written
for any coding agent (Claude Code, Codex, Gemini CLI, ...), and
`CLAUDE.md` is a pointer here. Human-facing rationale lives in
...
to a background tester, and continue
...
## Delegating test runs (cheap tokens!)

Routine test execution goes to a background tester on a cheaper
model, if your harness can delegate. The tester's brief: run the given
pytest selection with the diff summary, triage failures (real vs.
environmental vs. flaky), and report compactly; multiple testers may
run concurrently on disjoint selections. Harness-specific definitions
of the role live in the harness's own directory (Claude Code:
`.claude/agents/tester.md`); if your harness has no subagents, run
the tiers yourself in the same order — the protocol is the contract,
the delegation is an optimization. Use your own (expensive) cognition
for: writing tests,
...
`uv run pytest -m ai -q` (judgments; needs an agent CLI —
`cli/agent-run --check` tells you whether one is available),
...
`uv run mypy tests` all gate CI, and all three cover the tests
written here while skipping the ported trees
