...
filing deadlines from the statutes, and renders California court documents
— 28-line pleading paper, exhibits, and the official Judicial Council
forms — into filing-ready packets. An LLM operator drives it; the engine
does everything that has to be right. It is built by a law-office-study
student for use on one's own case — not a practice tool for lawyers, and
...
have. The machinery — `.git`, connector state, caches, build
intermediates — stays local and is never part of what you share.
(Today that is a Drive desktop client on macOS; [ROADMAP.md](ROADMAP.md)
Phase 6 replaces it with one-way sync over disjoint paths, which needs no
desktop client and works the same on Linux.)
