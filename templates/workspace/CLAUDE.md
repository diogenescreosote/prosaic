...
<!-- prosaic: this file is checked into prosaic at
     templates/workspace/CLAUDE.md and symlinked into the workspace, so
     there is one copy and it cannot drift. Edit it there. Rules only;
     rationale lives in prosaic's docs/ and design/adr/. -->

Each matter also has its own `CLAUDE.md` for what is specific to it —
