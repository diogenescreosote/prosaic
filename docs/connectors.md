...
| `mycase` | Walks a MyCase client portal's document folder tree, diffs against a manifest (doc id + content hash), downloads new/updated documents into staging renamed to dated snake_case, keeping the portal folder as a routing hint. With `billing: <dir>` configured, also pulls the Billing tab: every invoice and funds request has a first-class PDF export (`/bills/<id>.pdf`), which lands directly in `<dir>` (no triage — billing PDFs are born-digital and authoritatively named) as `<date>_invoice_<n>.pdf` / `<date>_funds_request_<ref>.pdf`, re-exported when a bill's listed status changes (a payment posts, a balance forwards) and never overwriting a previously fetched file. | Keychain |
